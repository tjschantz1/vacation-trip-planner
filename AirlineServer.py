...
#    columns = ['Airline_ID','AirlineName','FromCity','ToCity','Booked?'])
#
#cols = ['ResID','airlineID','Name', 'FromDate', 'ToDate']
#Reservations = pd.DataFrame(columns=cols)
#res_count = 0

#    # Upate airline booking Y/N
#    def UpdateAirlineBooking(self, airlineID, add_res):
#        global airlines
#        if add_res == True:
#            airlines['Booked?'] = np.where(airlines.Airline_ID == airlineID, 'Y', 
#                  airlines['Booked?'])
#        else:
#            airlines['Booked?'] = np.where(airlines.Airline_ID == airlineID, 'N', 
#                  airlines['Booked?'])
#        return True

#        # Update Y/N in airlines
#        if Reservations.airlineID[Reservations.airlineID==airlineID].count() == 1:
#            self.UpdateAirlineBooking(airlineID, False)
#            
#        # Remove reservation
#        Removed = Reservations[Reservations.ResID == ResID]
#        Reservations = Reservations[Reservations.ResID != ResID]

#        airlines = pd.DataFrame(list(self.collection.find()))
#        airlines = airlines.drop(['_id'], axis=1)
#        cols = ['Airline_ID','AirlineName','FromCity','ToCity','BookedYN']
#        airlines = airlines[cols] # reorder csolumns